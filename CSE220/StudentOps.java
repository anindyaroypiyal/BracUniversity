...
public class StudentOps extends Student{
  ...
  public  void addStudent(){
    ...
    int num = k.nextInt();
    k.nextLine();
    // using these pieces of information create a Student object
  ...
  public  void deleteStudent(){
    ...
    if (index != -1){
      ...
      // delete that object using left shift
    	for(int x = index;x<st.length-1;x++){
            st[x] = st[x+1];
          }
          st[st.length-1] = null;
          s--;
  ...
  public  void display(){
    for(int x= 0;x<s;x++){
       System.out.println(x +" display of x which was out of bounds");
      st[x].viewDetails();
    }
  }
  public  void resize(){
    //create a new array of bigger length
	  System.out.println(this.st.length+" bef");
    Student[] st = new Student[this.st.length+2];
    System.out.println(st.length+" aft");
    System.out.println("this . st "+ this.st[0] +"," + this.st[1] +"," +st[2]);
    for(int i = 0 ; i <= s ; i ++) {
    	st[i] = this.st[i];
}
    this.st = st;
